...
async function setupDatabase() {
  ...
  `);
  await dbClient.query(`
    ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS repechage_robot_ids JSONB DEFAULT '[]';
  `);
  await dbClient.query(`
    ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS repechage_advance_count INT DEFAULT 1;
  `);
  ...
  try {
    ...
    if (activeTournamentId) {
        const matchesRes = await dbClient.query(
            "SELECT * FROM matches WHERE tournament_id = $1 ORDER BY phase, round, group_label, id",
            [activeTournamentId]
        );
    ...
    const tournaments: Tournament[] = tournamentsRes.rows.map(row => {
      ...
      // NOVO: Carrega repechageRobotIds
      let repechageRobotIds: string[] = [];
      try {
        repechageRobotIds = row.repechage_robot_ids && Array.isArray(row.repechage_robot_ids) 
          ? row.repechage_robot_ids
          : JSON.parse(row.repechage_robot_ids || '[]');
      } catch {
        repechageRobotIds = [];
      }
      ...
      // O campo 'repechageWinner' foi removido daqui para ser calculado em checkAndGenerateGrandFinal.
    ...
    let currentTournament = tournaments.find(t => t.id === activeTournamentId);
...
function computeGroupTables(robots: Robot[] = state.robots, matches: Match[] = state.matches): Record<string, GroupTableItem[]> {
    ...
    // Filtra apenas matches de grupo (phase "groups") E repescagem (phase "repechage")
    const groupMatches = matches.filter((m) => m.phase === "groups" || m.phase === "repechage");
...
function generateRepechageMatches(): boolean {
  ...
  if (repechageParticipants.length < 2) {
  ...
  const baseMatches = generateGroupMatches(repechageGroup);
  
  const newMatches: Match[] = [];
  
  for (const match of baseMatches) {
      if (match.robotA?.id === 'bye' || match.robotB?.id === 'bye') {
        // Trata BYE (Folga) automaticamente como W.O. (Vitória por W.O.)
        const winner = match.robotA?.id !== 'bye' ? match.robotA : match.robotB;
        newMatches.push({
            id: uuidv4(),
            tournamentId: state.tournamentId,
            phase: "repechage", // Fase: Repechage
            round: match.round,
            group: 'R', // Grupo: 'R' para identificação da tabela de repescagem
            robotA: match.robotA?.id !== "bye" ? match.robotA : null,
            robotB: match.robotB?.id !== "bye" ? match.robotB : null,
            scoreA: winner?.id === match.robotA?.id ? 33 : 0,
            scoreB: winner?.id === match.robotB?.id ? 33 : 0,
            winner,
            finished: true, // Partida BYE é finalizada automaticamente
            type: "WO", // Vitória por W.O.
        } as Match);
      } else {
        // Partida normal de Round-Robin
        newMatches.push({
            id: uuidv4(),
            tournamentId: state.tournamentId,
            phase: "repechage",
...
function generateGroupEliminations() {
  ...
  if (newMatches.length > 0) {
    insertMatches(newMatches);
    return true; // Adicionado retorno para o controle de fluxo
  }
...
function checkAndGenerateGrandFinal(): boolean {
  ...
  // 1. FORÇA RE-CÁLCULO DAS TABELAS: Garante que os dados de Repescagem (Grupo 'R') e Grupos estejam frescos.
  const allMatchesInTournament = state.matches.filter(m => m.tournamentId === state.tournamentId);
  // Esta chamada recalcula todas as tabelas (Grupos A, B, ... e Repescagem 'R') com os dados mais recentes.
  state.groupTables = computeGroupTables(state.robots, allMatchesInTournament); 

  const allGroupLabels = Object.keys(state.groupTables || {});
  if (allGroupLabels.length === 0) return false;
  ...
  for (const g of eliminationGroupLabels) {
    ...
    if (gMatches.length === 0) {
        allGroupEliminationsFinished = false;
        break;
    }
  ...
  // CONDIÇÃO FINAL: Deve haver pelo menos 2 participantes para a chave final.
  if (champions.length < 2) return false;
  
  // Geração da Chave Final (o restante do código é mantido)
...
async function tryAdvanceToEliminationOrGrandFinal(): Promise<boolean> {
  if (!state.tournamentId) return false;

  // 1 — Recalcula tabelas
  const allMatches = state.matches.filter(m => m.tournamentId === state.tournamentId);
  state.groupTables = computeGroupTables(state.robots, allMatches);

  const tournament = state.tournaments.find(t => t.id === state.tournamentId);
  if (!tournament) return false;

  // 🔥 CONVERSÃO CORRETA DO VALOR
  const useRepechage = tournament.useRepechage === true;

  // Grupos normais (exclui repescagem)
  const eliminationGroups = Object.keys(state.groupTables || {}).filter(g => g !== "R");
  const hasGroups = eliminationGroups.length > 0;

  // === 2 — ELIMINAÇÃO DOS GRUPOS ===
  let isGroupEliminationFinished = !hasGroups;

  const hasGroupEliminations = state.matches.some(
    m => m.tournamentId === state.tournamentId && m.phase === "elimination" && m.group !== null
  );

  if (hasGroups) {
    const groupMatches = state.matches.filter(
      m => m.tournamentId === state.tournamentId && m.phase === "groups"
    );

    // A) Não existem partidas de eliminação ainda → tenta gerar
    if (!hasGroupEliminations) {
      const allDone = groupMatches.length > 0 && groupMatches.every(m => m.finished);
      if (allDone) {
        if (generateGroupEliminations()) {
          console.log("➡️ Geração da eliminação interna dos grupos feita.");
          return true;
        }
      }
    }

    // B) Tenta avançar WOs + próximos rounds
    if (await progressGroupEliminations()) return true;

    // C) Se não há mais o que gerar, verifica finalização completa dos grupos
    updateGroupChampions();

    const eliminationMatches = state.matches.filter(
      m =>
        m.tournamentId === state.tournamentId &&
        m.phase === "elimination" &&
        m.group !== null
    );

    isGroupEliminationFinished =
      hasGroupEliminations &&
      eliminationMatches.length > 0 &&
      eliminationMatches.every(m => m.finished);
  }

  // === 3 — TRATAMENTO DA REPESCAGEM ===
  // 🔥 AQUI ESTÁ A CORREÇÃO PRINCIPAL:
  // Se o torneio NÃO usa repescagem, já considera como concluída.
  let isRepechageRequiredAndCompleted = !useRepechage;

  if (useRepechage) {
    const repeMatches = state.matches.filter(
      m => m.tournamentId === state.tournamentId && m.phase === "repechage"
    );

    if (repeMatches.length > 0 && repeMatches.every(m => m.finished)) {
      isRepechageRequiredAndCompleted = true;
    }
  }

  // === 4 — CONDIÇÃO PARA GERAR A FINAL ===
  if (isGroupEliminationFinished && isRepechageRequiredAndCompleted) {
    if (checkAndGenerateGrandFinal()) {
      console.log("🏆 Final do torneio gerada.");
      return true;
    }
    ...
    if (!finalExists) {
      ...
      console.log("🎉 Torneio finalizado automaticamente.");
      return true;
    }
  }
  ...
  console.log(`📌 DEBUG:
    ...
    - isGroupEliminationFinished = ${isGroupEliminationFinished}
    - isRepechageRequiredAndCompleted = ${isRepechageRequiredAndCompleted}
  `);
...
async function progressGroupEliminations(): Promise<boolean> {
  ...
  if (newMatches.length > 0) {
    ...
    await insertMatches(newMatches); // AWAIT ADICIONADO
...
async function finalizeMatch(id: string, scoreA: number, scoreB: number, type: 'normal' | 'KO' | 'WO' = 'normal') {
  ...
  if (currentMatchInState.phase === "groups") {
    const allGroupMatches = state.matches.filter(x => x.tournamentId === currentMatchInState.tournamentId && x.phase === "groups");
  ...
  // C. Se a partida finalizada é de eliminação (interna de grupo ou Fase Final Geral)
        ...
        // 1. Encontra todos os matches no round e grupo atual.
        const matchesInCurrentRound = state.matches.filter(x => 
            x.phase === "elimination" && 
            x.round === round && 
            x.group === group && // Pode ser um grupo ('A', 'B', ...) ou null (Fase Final Geral)
            x.tournamentId === currentMatchInState.tournamentId
        );
        
        // 2. Se a rodada atual do grupo/fase terminou, PROGRIDE os vencedores
...
app.post("/tournaments", async (req, res) => {
  ...
  // 🔥 Normalização correta da repescagem
  let useRepechage = false;
  if (req.body.useRepechage !== undefined) {
    const raw = String(req.body.useRepechage).toLowerCase();
    useRepechage = ["true", "1", "yes", "on"].includes(raw);
  }
  ...
  try {
    await dbClient.query(
      ...
      [
        ...
        '[]',
        '[]',
        repechageAdvanceCount,
        useRepechage // <-- SALVA CERTO
      ]
    );
...
app.put("/tournaments/:id", async (req, res) => {
  ...
  const current = state.tournaments.find(t => t.id === tournamentId);
  if (!current)
    return res.status(404).json({ error: "Torneio não encontrado." });

  if (current.status !== "draft")
    return res.status(403).json({ error: "Somente torneios em draft podem ser editados." });
  ...
  if (image !== undefined) { updates.push(`image = $${idx++}`); values.push(image || null); }
  if (groupCount !== undefined) { updates.push(`group_count = $${idx++}`); values.push(Number(groupCount)); }
  if (advancePerGroup !== undefined) { updates.push(`advance_per_group = $${idx++}`); values.push(Number(advancePerGroup)); }
  if (repechageAdvanceCount !== undefined) { updates.push(`repechage_advance_count = $${idx++}`); values.push(Number(repechageAdvanceCount)); }

  // 🔥 Conversão correta para boolean
  if (useRepechage !== undefined) {
    const normalized =
      typeof useRepechage === "string"
        ? ["true", "1", "yes", "on"].includes(useRepechage.toLowerCase())
        : !!useRepechage;

    updates.push(`use_repechage = $${idx++}`);
...
// NOVO POST: Adicionar/Remover Robôs (Apenas Draft ou Grupos Finalizados) para REPESCAGEM
app.post("/tournaments/:id/set-repechage-robots", async (req, res) => {
    const { robotIds } = req.body; // Array de IDs de robôs
    const tournamentId = req.params.id;
    ...
    // Permite definir se o status for 'draft' ou 'active'
    if (currentTour.status !== 'draft' && currentTour.status !== 'active') {
        return res.status(403).json({ error: "Apenas torneios em status 'draft' ou 'active' podem ter a lista de repescagem alterada." });
    }
    
    if (!Array.isArray(robotIds)) return res.status(400).json({ error: "Lista de IDs de robôs inválida." });
    
    // Opcional: Adicionar validação se a fase de repescagem já começou.
    const repechageStarted = state.matches.some(m => m.tournamentId === tournamentId && m.phase === 'repechage');
    if (repechageStarted) {
        return res.status(403).json({ error: "A repescagem já foi iniciada, não é possível alterar a lista." });
    }
    
    // Filtra IDs inválidos e converte o array para string JSON para salvar no DB
    const validRobotIds = robotIds.filter((id: string) => state.robots.some(r => r.id === id));
    const robotIdsJson = JSON.stringify(validRobotIds);

    await dbClient.query(
        `UPDATE tournaments SET repechage_robot_ids = $1 WHERE id = $2`,
        [robotIdsJson, tournamentId]
    );

    await loadStateFromDBAndBroadcast();
    res.json({ ok: true, message: `Lista de participantes da repescagem atualizada para ${validRobotIds.length} robôs.` });
});
