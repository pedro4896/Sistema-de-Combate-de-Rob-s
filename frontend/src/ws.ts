...
ws.onmessage = (e) => { const msg: Msg = JSON.parse(e.data); listeners.forEach(cb=>cb(msg)); };

export function onMessage(cb: (m: Msg) => void) { listeners.add(cb); return () => listeners.delete(cb); }
